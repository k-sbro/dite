...
## Active Design Spec
**반드시 구현 전에 아래 문서를 읽고 참조할 것:**
- `docs/superpowers/specs/2026-04-16-meal-tracker-design.md`

이 문서에는 다음이 정의되어 있다:
- 전체 아키텍처 및 페이지 구조
- DB 스키마 (meals 테이블, meal-photos Storage)
- 컴포넌트 구조 및 폴더 레이아웃
- Server Actions 목록 및 역할
- Gemini 호출 흐름 및 프롬프트
- 에러 처리 전략
- 인프라 연동 계획 (Git, Supabase, Vercel)
- Acceptance Criteria

**스펙과 다른 결정을 내릴 경우, 반드시 스펙 문서를 먼저 업데이트하고 구현한다.**
...
# git, supabase, vercel
- after major code write, commit and push.# Next.js + Supabase Development Rules
